...
app/
  ...
  auth/sign-out/      # POST → clears session cookie
src/
  components/
  ...
  lib/
    ...
    realtime/         # useRealtimeChannel seam — polling fallback now, Supabase Realtime later
tests/                # Vitest setup + permissions + date format
e2e/                  # Playwright smoke
```

## Data layer

Every page reads from `await dataProvider()` and pulls `queries` / `mutations`. The current implementation is in-memory — fixtures live in `src/lib/data/fixtures/*`. To swap in Supabase, replace `src/lib/data/mock-impl.ts` with a real client; `dataProvider()` is the only callsite.

## Auth

Today `getCurrentUser()` reads the `ngn_persona` cookie. The dev switcher (`/dev/persona`) sets it. Replace the cookie read with a Supabase Auth session lookup and `getCurrentUser` stays the same shape.
